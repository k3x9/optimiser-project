...
router.post("/search", async (req, res) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      getData().then(() => {
          ...
          fetch(apiEndpoint, {
              ...
              const answer = {'Name': [], 'Location': [], 'Rating': [], 'Money': [], 'Day': [], 'Time': [], 'TravelDuration': []};
              let cnt = 0;
              for(let i = 0; i < indices.length && cnt < 10; i++){
                  if(result[indices[i]]['travelDuration'] <= time_req && data[indices[i]][money_column] <= money_left){
                      ...
                      answer['Day'].push(data[indices[i]][4]);
  ...
  try {
    await go(req.body.Location, req.body.Time, req.body.Money).then((final_answer) => {
      res.json({ success: 'Done6' , result: JSON.stringify(final_answer)});
    });
  } catch (error) {
    console.error(error);
