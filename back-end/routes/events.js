...
router.post("/search", async (req, res) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      getData().then(() => {
          ...
          fetch(apiEndpoint, {
              ...
              const answer = {'Name': [], 'Type': [], 'Location': [], 'Rating': [], 'Money': [], 'Date': [], 'Time': [], 'TravelDuration': []};
              let cnt = 0;
  ...
  try {
    await go(req.body.Location, req.body.Time, req.body.Money).then((final_answer) => {
      res.json({ success: 'Done5' , result: JSON.stringify(final_answer)});
    });
  } catch (error) {
    console.error(error);
