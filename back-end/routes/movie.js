...
router.post("/search", async (req, res) => {
  ...
  async function go(op1, op2, op3){
    return new Promise((resolve, reject) => {
      const location_url = op3;
      const time_req = op2;
      const money_left = op1;
      ...
      getData().then(() => {
          ...
          fetch(apiEndpoint, {
              ...
              const answer = {'Name': [], 'Location': [], 'Date': [], 'Rating': [], 'Money': [], 'Time': [], 'TravelDuration': []};
              let cnt = 0;
              for(let i = 0; i < indices.length && cnt < 10; i++){
                  if(result[indices[i]]['travelDuration'] <= time_req && data[indices[i]][money_column] <= money_left){
                      ...
                      answer['Date'].push(data[indices[i]][2]);
                      answer['Rating'].push(data[indices[i]][3]);
                      answer['Money'].push(data[indices[i]][4]);
                      answer['Time'].push(data[indices[i]][5]);
  ...
  try {
    await go(req.body.Money, req.body.Time, req.body.Location).then((final_answer) => {
      res.json({ success: 'Done' , result: JSON.stringify(final_answer)});
    });
  } catch (error) {
    console.error(error);
